...
export class ItadService {
    ...
    async handleGamesSync() {
        try {
            ...
            const apiKey = TrophiesConfig.itad.apiKey;
    ...
    async searchGameByName(gameName: string): Promise<Partial<Game[]> | null> {
        try {
            const gamesResponse: any = await axios.get(`https://api.isthereanydeal.com/games/search/v1?key=${TrophiesConfig.itad.apiKey}&title=${gameName}`);
